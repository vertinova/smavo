  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <p className="text-[11px] text-muted uppercase tracking-widest mb-1">Inventaris</p>
          <h2 className="text-xl font-bold text-foreground tracking-tight">Aset Sekolah</h2>
        </div>
            ...
            <tbody>
              {isLoading ? [...Array(5)].map((_, i) => (
                ...
                <tr><td colSpan={8} className="px-4 py-16 text-center">
                  <Package size={36} className="mx-auto text-muted/30 mb-3" />
                ...
                <tr key={a.id} className="tr">
                  ...
                  <td className="td">
                    <div className="flex gap-0.5 justify-end">
                      <button onClick={() => showQr(a.id)} className="p-1.5 rounded-lg hover:bg-foreground/[0.04] text-muted hover:text-foreground transition-colors" title="QR Code">
                        <QrCode size={15} />
                      </button>
                      <button onClick={() => openEdit(a)} className="p-1.5 rounded-lg hover:bg-foreground/[0.04] text-muted hover:text-foreground transition-colors" title="Edit">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => setDelId(a.id)} className="p-1.5 rounded-lg hover:bg-danger-muted text-muted hover:text-danger transition-colors" title="Hapus">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn" onClick={close}>
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-modal w-full max-w-2xl animate-slideUp" onClick={e => e.stopPropagation()}>
      ...
      {qrUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn" onClick={() => setQrUrl(null)}>
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md" />
          <div className="relative bg-card border border-border rounded-2xl shadow-modal w-full max-w-xs p-6 text-center animate-slideUp" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold text-foreground mb-4">QR Code Aset</h3>
            <img src={qrUrl} alt="QR Code" className="w-48 h-48 mx-auto mb-4 rounded-xl" />
