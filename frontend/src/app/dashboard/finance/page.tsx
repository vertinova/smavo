  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground tracking-tight">Keuangan BOS</h2>
          <p className="text-sm text-muted mt-0.5">Kelola anggaran dan pengeluaran dana BOS</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {[
          { label: 'Total Anggaran', value: budget, icon: Wallet, color: 'bg-amber-500', ring: 'ring-amber-500/10' },
          { label: 'Terpakai', value: spent, icon: TrendingDown, color: 'bg-rose-500', ring: 'ring-rose-500/10' },
          { label: 'Sisa Anggaran', value: remaining, icon: CheckCircle, color: 'bg-emerald-500', ring: 'ring-emerald-500/10' },
        ].map(c => (
          <div key={c.label} className="card">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-9 h-9 rounded-xl ${c.color} flex items-center justify-center ring-4 ${c.ring}`}>
                <c.icon size={16} className="text-white" />
              </div>
              <span className="text-xs font-medium text-muted uppercase tracking-wide">{c.label}</span>
            </div>
        ...
        {(['rkas', 'expenses'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === t ? 'bg-white text-foreground shadow-card' : 'text-muted hover:text-foreground'
            }`}>
                ...
                {rkasData?.data?.items?.map((item: any) => {
                  ...
                  return (
                    <tr key={item.id} className="tr">
                      ...
                      <td className="td text-right text-emerald-600 font-medium">{formatCurrency(b - s)}</td>
                      <td className="td">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                            <div className={`h-1.5 rounded-full transition-all ${pct > 80 ? 'bg-rose-500' : 'bg-primary'}`}
