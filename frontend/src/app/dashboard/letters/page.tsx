...
const TYPES: Record<string, { label: string; desc: string; color: string; bg: string }> = {
  SURAT_TUGAS: { label: 'Surat Tugas', desc: 'Penugasan guru/staf untuk kegiatan tertentu', color: 'text-accent', bg: 'bg-accent/10' },
  SURAT_KETERANGAN_AKTIF: { label: 'SK Aktif', desc: 'Bukti status siswa aktif', color: 'text-success', bg: 'bg-success/10' },
  SURAT_KETERANGAN_PINDAH: { label: 'SK Pindah', desc: 'Proses mutasi siswa', color: 'text-warning', bg: 'bg-warning/10' },
  SURAT_KETERANGAN_LULUS: { label: 'SK Lulus', desc: 'Bukti kelulusan siswa', color: 'text-info', bg: 'bg-info/10' },
  SURAT_IZIN: { label: 'Surat Izin', desc: 'Izin kegiatan atau permohonan', color: 'text-accent', bg: 'bg-accent/10' },
  LAINNYA: { label: 'Lainnya', desc: 'Surat kustom lainnya', color: 'text-muted-foreground', bg: 'bg-foreground/5' },
};
...
export default function LettersPage() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        <p className="text-[11px] text-muted uppercase tracking-widest mb-1">Administrasi</p>
        <h2 className="text-xl font-bold text-foreground tracking-tight">Persuratan</h2>
      </div>
        ...
        {Object.entries(TYPES).map(([key, val]) => (
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`w-8 h-8 rounded-lg ${val.bg} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}>
                <FileText size={14} className={val.color} />
              </div>
              <div>
      ...
      <div className="card">
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => <div key={i} className="h-14 bg-foreground/[0.03] rounded-lg animate-pulse" />)}
          </div>
        ) : letters.length === 0 ? (
          <div className="text-center py-10">
            <FileText size={36} className="mx-auto text-muted/30 mb-3" />
            <p className="text-muted text-sm">Belum ada surat dibuat</p>
          </div>
        ) : (
          <div className="space-y-2">
            {letters.map((l: any) => (
              <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-foreground/[0.03] transition-colors group">
                <div className={`w-8 h-8 rounded-lg ${TYPES[l.letterType]?.bg || 'bg-accent/10'} flex items-center justify-center shrink-0`}>
                  <FileText size={14} className={TYPES[l.letterType]?.color || 'text-accent'} />
                </div>
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-danger-muted text-muted hover:text-danger transition-all"
                >
                  <Trash2 size={14} />
                </button>
              </div>
